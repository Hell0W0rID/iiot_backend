...
func (u *UpdateScheduleJobRequest) Validate() error {
	...
	if u.ScheduleJob.Definition != nil {
		...
		if err != nil {
			return errors.NewCommonIIOT(errors.KindContractInvalid, "invalid ScheduleDef.", err)
		}
	}
	...
	if u.ScheduleJob.Actions != nil {
		for _, action := range u.ScheduleJob.Actions {
			err = action.Validate()
			if err != nil {
				return errors.NewCommonIIOT(errors.KindContractInvalid, "invalid ScheduleAction.", err)
			}
		}
	}
