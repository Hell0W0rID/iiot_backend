...
// IIOT provides an abstraction for all internal IIOT errors.
...
// CommonIIOT generalizes an error structure which can be used for any type of IIOT error.
type CommonIIOT struct {
...
func (ce CommonIIOT) Error() string {
        if ce.err == nil {
                return ce.message
        }

        // ce.err.Error functionality gets the error message of the nested error and which will handle both CommonIIOT
...
func (ce CommonIIOT) DebugMessages() string {
        if ce.err == nil {
                return ce.callerInfo + ": " + ce.message
        }

        if w, ok := ce.err.(CommonIIOT); ok {
...
// Message returns the first level error message without further details.
func (ce CommonIIOT) Message() string {
...
// Code returns the status code of this error.
func (ce CommonIIOT) Code() int {
        return ce.code
}

// Kind returns the error kind of this error.
func (ce CommonIIOT) Kind() string {
...
// of wrapped errors.
func (ce CommonIIOT) Unwrap() error {
        return ce.err
}

// Is determines if an error is of type CommonIIOT.
...
func (ce CommonIIOT) Is(err error) bool {
        switch err.(type) {
        case CommonIIOT:
                return true
        default:
...
// NewCommonIIOT creates a new CommonIIOT with the information provided.
func NewCommonIIOT(kind ErrKind, message string, wrappedError error) CommonIIOT {
        return CommonIIOT{
                kind:       kind,
...
// NewCommonIIOTWrapper creates a new CommonIIOT to wrap another error to record the function call stacks.
func NewCommonIIOTWrapper(wrappedError error) CommonIIOT {
...
// function is a helper function which eliminates the need for the 'callerInfo' field in the `CommonIIOT` type and
// providing an 'callerInfo' string when creating an 'CommonIIOT'
...
// KindMapping determines the correct IIOT error kind for the given HTTP response code.
