...
import (
	...
	httpClient "iiot-backend/pkg/go-mod-core-contracts/clients/http"
	...
	msgTypes "iiot-backend/pkg/go-mod-messaging/pkg/types"

	"iiot-backend/pkg/go-mod-configuration/pkg/types"
)

const (
	keeperTopicPrefix = "iiot/configs"
	KeyDelimiter      = "/"
)
...
func (k *keeperClient) GetConfiguration(configStruct interface{}) (interface{}, error) {
	...
	if !exists {
		return nil, fmt.Errorf("the Configuration service (IIOT Keeper) doesn't contain configuration for %s", k.configBasePath)
	}
...
func (k *keeperClient) WatchForChanges(updateChannel chan<- interface{}, errorChannel chan<- error, configuration interface{}, waitKey string, getMsgClientCb func() messaging.MessageClient) {
	...
	go func() {
		...
		// refer to the isFirstUpdate variable declared in https://iiot-backend/pkg/go-mod-bootstrap/blob/main/bootstrap/config/config.go
		updateChannel <- nil
	...
	outerLoop:
		for {
			...
			case msgEnvelope := <-messages:
				...
				// e.g. keyPrefix = "iiot/3.0/iiot-data/Writable" which is the root level of Writable configuration
