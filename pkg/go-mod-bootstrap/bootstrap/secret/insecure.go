...
func NewInsecureProvider(config interfaces.Configuration, lc logger.LoggerClient, dic *di.Container) *InsecureProvider {
...
// RetrieveSecret retrieves secrets from a Insecure Secrets secret store.
...
func (p *InsecureProvider) RetrieveSecret(secretName string, keys ...string) (map[string]string, error) {
...
// SaveSecret attempts to store the secrets in the ConfigurationProvider's InsecureSecrets. If no ConfigurationProvider
...
func (p *InsecureProvider) SaveSecret(secretName string, secrets map[string]string) error {
	configClient := container.ConfigClientFrom(p.dic.Get)
	if configClient == nil {
		return errors.NewCommonIIOT(errors.KindNotAllowed, "can't store secrets. ConfigurationProvider is not in use or has not been properly initialized", nil)
	}
	...
	if err != nil {
		return errors.NewCommonIIOT(errors.KindCommunicationError, "error setting secretName value in the config provider", err)
	}
	...
	for key, value := range secrets {
		...
		if err != nil {
			return errors.NewCommonIIOT(errors.KindCommunicationError, "error setting secretData key/value pair in the config provider", err)
		}
	}
	return nil
}
