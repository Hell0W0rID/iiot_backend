...
import (
	...
	"iiot-backend/pkg/go-mod-bootstrap/bootstrap/file"
	"iiot-backend/pkg/go-mod-bootstrap/bootstrap/utils"
...
func (cp *Processor) Process(
	...
	if useProvider {
		...
		cp.listenForCommonChanges(serviceConfig, cp.commonConfigClient, cp.privateConfigClient, utils.BuildBaseKey(configStem, common.CoreCommonConfigServiceName, allServicesKey))
		cp.lc.Infof("listening for all services common config changes")
		if cp.appConfigClient != nil {
			cp.listenForCommonChanges(serviceConfig, cp.appConfigClient, cp.privateConfigClient, utils.BuildBaseKey(configStem, common.CoreCommonConfigServiceName, appServicesKey))
			cp.lc.Infof("listening for application service common config changes")
		}
		if cp.deviceConfigClient != nil {
			cp.listenForCommonChanges(serviceConfig, cp.deviceConfigClient, cp.privateConfigClient, utils.BuildBaseKey(configStem, common.CoreCommonConfigServiceName, deviceServicesKey))
...
func (cp *Processor) loadCommonConfig(
	...
	cp.commonConfigClient, err = createProvider(cp.lc, utils.BuildBaseKey(common.CoreCommonConfigServiceName, allServicesKey), configStem, configProviderInfo.ServiceConfig())
	if err != nil {
	...
	commonConfigReadyPath := fmt.Sprintf("%s/%s/%s", configStem, common.CoreCommonConfigServiceName, config.CommonConfigDone)
	...
	case config.ServiceTypeApp:
		serviceTypeSectionKey = utils.BuildBaseKey(common.CoreCommonConfigServiceName, appServicesKey)
	...
	case config.ServiceTypeDevice:
		serviceTypeSectionKey = utils.BuildBaseKey(common.CoreCommonConfigServiceName, deviceServicesKey)
		cp.lc.Infof("loading the common configuration for service type %s", serviceType)
		serviceTypeConfig, err = copyConfigurationStruct(serviceConfig)
		if err != nil {
...
func GetConfigFileLocation(lc logger.LoggerClient, flags flags.Common) string {
