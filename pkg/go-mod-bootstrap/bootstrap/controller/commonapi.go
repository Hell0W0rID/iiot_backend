...
// AddSecret handles the request to the /secret endpoint. Is used to add IIOT Service exclusive secret to the Secret Store
// It returns a response as specified by the API swagger in the openapi directory
...
// addSecret adds IIOT Service exclusive secret to the Secret Store
func addSecret(dic *di.Container, request commonDTO.SecretRequest) errors.IIOT {
	secretName, secret := prepareSecret(request)

	secretProvider := container.SecretProviderFrom(dic.Get)
	if secretProvider == nil {
		return errors.NewCommonIIOT(errors.KindServerError, "secret provider is missing. Make sure it is specified to be used in bootstrap.Run()", nil)
	}

	if err := secretProvider.SaveSecret(secretName, secret); err != nil {
		return errors.NewCommonIIOT(errors.Kind(err), "adding secret failed", err)
	}
	return nil
}
