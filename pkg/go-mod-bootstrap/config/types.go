...
// ServiceInfo contains configuration settings necessary for the basic operation of any IIOT service.
...
func NewSecretStoreSetupClientInfo() *ClientsCollection {
	secretStoreStepClient := ClientsCollection{
		common.SecuritySecretStoreSetupServiceName: &ClientInfo{
			Host:     "localhost",
			Port:     59843,
			Protocol: "http",
		}}
...
func NewSecretStoreInfo(serviceKey string) SecretStoreInfo {
	return SecretStoreInfo{
		...
		TokenFile:               fmt.Sprintf("/tmp/iiot/secrets/%s/secrets-token.json", serviceKey),
		...
		RuntimeTokenProvider: types.RuntimeTokenProviderInfo{
			...
			Host:            "localhost",
			Port:            59841,
			TrustDomain:     "iiotfoundry.org",
			EndpointSocket:  "/tmp/iiot/secrets/spiffe/public/api.sock",
...
// MessageBusInfo provides parameters related to connecting to the IIOT MessageBus
type MessageBusInfo struct {
	// Disabled indicates if the use of the IIOT MessageBus is disabled.
	Disabled bool
...
type ExternalMQTTInfo struct {
	...
	// TODO: move PublishTopic and SubscribeTopics to Topics in IIOT 3.0
