...
func SubscribeCommandRequests(ctx context.Context, requestTimeout time.Duration, dic *di.Container) errors.IIOT {
	lc := bootstrapContainer.LoggerClientFrom(dic.Get)
...
func processDeviceCommandRequest(
	...
	internalResponseTopic := common.BuildTopic(baseTopic, common.ResponseTopic, common.CoreCommandServiceName, requestEnvelope.RequestID)
...
func SubscribeCommandQueryRequests(ctx context.Context, dic *di.Container) errors.IIOT {
	lc := bootstrapContainer.LoggerClientFrom(dic.Get)
	baseTopic := container.ConfigurationFrom(dic.Get).MessageBus.GetBaseTopicPrefix()
...
func processCommandQueryRequest(
	...
	// internal response topic scheme: <ResponseTopicPrefix>/<service-name>/<request-id>
	internalQueryResponseTopic := common.BuildTopic(baseTopic, common.ResponseTopic, common.CoreCommandServiceName, requestEnvelope.RequestID)
